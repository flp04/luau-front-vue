    ...
    <main id="main" class="main" v-if="!carregando">
      ...
      <div class="card">
        <div class="card-body">
          ...
          <h5 class="card-title">Crediario</h5>
              ...
              <table class="table table-borderless">
                ...
                <tbody>
                  <tr v-for="(item, index) in formulario">
                    ...
                    <td><select :disabled="!item.tamanhos" id="tamanho" class="form-select" required v-model="item.tamanho_id">
                      <option value="">- Selecione -</option>
    ...
    methods: {
      ...
      calcularFormulario(index) {
        ...
        if (formulario.preco_venda != '' && formulario.quantidade != '') {
          ...
          this.total.toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,')
        }
      },
      selecionarProduto(produto_id, index) {
        console.log(produto_id, index)
        if (produto_id > 0) {
