    ...
    <main id="main" class="main">
      <div class="pagetitle">
          ...
          <ol class="breadcrumb">
              ...
              <li class="breadcrumb-item">{{ produto.id }}</li>
          </ol>
          </nav>
      </div><!-- End Page Title -->
      ...
      <div class="card">
        <div class="card-body">
          <div class="row">
            <div class="col-md-6">
              <h5 v-if="!editarForm" class="card-title">#{{ produto.id }} {{ produto.nome }}</h5>
              <h5 v-else class="card-title"><input type="text" class="form-control" v-model="produto.nome"></h5>
            </div>
            <div class="col-md-6">
              <div class="d-flex justify-content-end">
                ...
                <!-- <a v-if="!editarForm" style="margin-top: 15px; margin-right: 20px;" class="btn btn-danger" href:"/nova-compra">Comprar</a> -->
                <button v-if="editarForm" style="margin-top: 15px; margin-right: 20px;" class="btn btn-danger" @click="editar(false)">Cancelar</button>
                <button v-if="editarForm" style="margin-top: 15px; margin-right: 20px;" class="btn btn-primary" @click="atualizarProduto()">Salvar</button>
              </div>
            </div>
          </div>
          <form v-if="editarForm">
            <div class="row g-3">
              <div class="col-md-6">
                ...
                <!-- <input type="text" :disabled="!editarForm" v-model="produto.categoria" class="form-control"> -->
          ...
          <form v-else>
            <div class="row g-3">
              ...
              <div class="col-md-6">
                <label for="tecido" class="form-label fw-bold">Tecido</label>
                <input type="text" :disabled="!editarForm" v-model="produto.tecido" class="form-control">
              </div>
            </div>
            <br>
            <div class="row g-3">
...
export default {
  ...
  methods: {
    pegarProduto() {
      let url = `http://127.0.0.1:8000/api/produto/${this.id}`
      fetch(url, {
        method: "GET",
        headers: {
      ...
      .then(data => {
          console.log(data);
          this.produto = data
          this.produto.categoria = data.categoria.nome
